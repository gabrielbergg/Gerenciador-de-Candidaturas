...
public class UserLoginController {
    ...
    public ResponseEntity<?> register(@RequestBody RegistroDto body) {
        ...
        if(user.isEmpty()) {
            ...
            return ResponseEntity.ok(new ResponseDto( newUser.getLogin(), newUser.getRoles(), token, newUser.getId()));
        }
    ...
    public ResponseEntity<?> login(@RequestBody @Valid AutheticationDto body) {
        ...
        if(passwordEncoder.matches(body.password(), user.getPassword())) {
            ...
            return ResponseEntity.ok(new AutheticationDto(user.getId(), user.getName(), user.getLogin(), user.getRoles(), token));
        }
        return ResponseEntity.badRequest().build();
    }
